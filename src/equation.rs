    ...
    pub mod hyperbolic {
        pub mod dirichlet {
                ...
                F5: Fn(T, T) -> T,
            {
                ...
                let u_km1_j_arg = one - beta * t_step / alpha;

                let u_kp1_jp1_arg = (a * t_step * t_step / alpha / x_step / x_step
                    + b * t_step * t_step / alpha / two / x_step)
                    / u_km1_j_arg;
                let u_kp1_j_arg = (-one - two * a * t_step / alpha / x_step / x_step
                    + c * t_step * t_step / alpha)
                    / u_km1_j_arg;
                let u_kp1_jm1_arg = (a * t_step * t_step / alpha / x_step / x_step
                    - b * t_step * t_step / alpha / two / x_step)
                    / u_km1_j_arg;
                let u_k_j_arg = two / u_km1_j_arg;
                let free_arg = |x: T, t: T| d(x, t) * t_step * t_step / alpha / u_km1_j_arg;
                ...
                for k in 0..(nt - 2) {
                    ...
                    for j in 1..(nx - 1) {
                        ...
                        d[j][0] = d[j][0] - u_k_j_arg * u[k + 1][j] - free_arg(x, t);
                        m.push(row);
                    }
